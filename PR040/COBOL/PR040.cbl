             ...
             10 WS-COU-VALUES.
                ...
                15 WS-COU-ADDRESS-TABLE.
                   20 WS-COU-ADDRESS    PIC X(30)
                         OCCURS 3 TIMES.
                15 WS-COU-CONTACT-PERSON
                                        PIC X(30).
                15 WS-COU-CONTACT-NUMBER
                                        PIC X(12).
       01 WS-ST-TABLE.
       ...
       01 WS-REPORT-HEADER.
          ...
          05 WS-ORDER-HEADING.
             ...
             10 WS-ORDER-HEADING-LINE-THREE.
                ...
                15 WS-ORDER-DESTINATION.
                   20 WS-ORDER-DESTINATION-CITY
                                        PIC X(4)    VALUE SPACES.
                   20 WS-ORDER-DESTINATION-STATE
          ...
          05 WS-REPORT-ITEM-COLUMNS.
             10 WS-REPORT-ITEM-HEADINGS.
                15 WS-REPORT-ITEM-CODE-HEADING
                                        PIC X(9)
                                                    VALUE
                      "ITEM CODE".
                15 FILLER               PIC X(4)
                ...
                15 FILLER               PIC X(4)
                                                    VALUE
                      SPACES.
                15 WS-REPORT-NO-OF-BOXES-HEADING
                                        PIC X(15)
                                                    VALUE
                      "NUMBER OF BOXES".
             10 WS-REPORT-ITEM-VALUES.
                15 WS-REPORT-ITEM-CODE-VALUE
                                        PIC X(5).
                15 FILLER               PIC X(7)    VALUE SPACES.
                15 WS-REPORT-QUANTITY-VALUE
                                        PIC ZZZZ9.
                15 FILLER               PIC X(7)    VALUE SPACES.
                15 WS-REPORT-NO-OV-BOXES-VALUE
                                        PIC ZZZZ9.
       01 WS-REPORT-FOOTER.
      ...
      *       -  LINE ITEMS
      *       -  ADVANCE RECORDS
           PERFORM B000-PROCESS UNTIL WS-EOF-DLV EQUAL TO "Y".
      *    PROGRAM CLEANUP
      *       -  CLOSE FILES
      *       -  END PROGRAM
      ...
      *    MOVE N TO PREVENT INF READING LOOP
           MOVE "N" TO DLV-PACKED
      
      *    REPORT STUFF
           PERFORM B200-PROCESS-AND-WRITE-REPORT.

      *    PREP FOR NEXT RECORD
           PERFORM R100-READ-DLV.

      *    LOOKS TO SEE IF THERE IS A CHANGE IN THE KEY AND SETS CHANGE
      *    FLAGS ACCORDINGLY
       ...
       B200-PROCESS-AND-WRITE-REPORT.
      *    WRITES OUT FOOTERS
      *    SYNCS THE STATE AND COURIER ONLY IF STATE CHANGES
      *    UPDATES THE BRANCH INFO IF THE BRANCH CHANGES
           EVALUATE TRUE
           WHEN WS-STATE-CHANGE-FLAG EQUAL TO "Y"
                PERFORM S100-SYNC-ST-TABLE
                 
                PERFORM W300-WRITE-BRANCH-FOOTER
                MOVE ZEROS TO WS-BRN-BOX-COUNT
                PERFORM S200-SYNC-BRN-REC
                 
                PERFORM W400-WRITE-CITY-FOOTER
                MOVE ZEROS TO WS-CITY-BOX-COUNT 
           WHEN WS-CITY-CHANGE-FLAG EQUAL TO "Y"
                PERFORM W300-WRITE-BRANCH-FOOTER
                MOVE ZEROS TO WS-BRN-BOX-COUNT
                PERFORM S200-SYNC-BRN-REC
                ...
                PERFORM W400-WRITE-CITY-FOOTER
                MOVE ZEROS TO WS-CITY-BOX-COUNT 
           WHEN WS-BRANCH-CHANGE-FLAG EQUAL TO "Y"
                PERFORM W300-WRITE-BRANCH-FOOTER 
                MOVE ZEROS TO WS-BRN-BOX-COUNT
                PERFORM S200-SYNC-BRN-REC
           END-EVALUATE.

      *    WRITES OUT NEW HEADER IF THE KEYS HAVE CHANGES OR IF THE MAX
      ...
      *    WRITES OUT A DLV-ITEM AFTER ALL FOOTER AND HEADER LOGIC HAS
      *    OCCURED. ALSO ADDS TO THE BOX COUNT FOR BRN AND CITY. 
      *    SHOULD ALWAYS BE WRITTEN OUT AFTER THE FOOTERS AND 
      *    HEADERS HAVE BEEN WRITTEN OR NOT (FOOTERS AND HEADERS SHOULD
      *    COME BEFORE ITEMS)
      ...
      *    UPDATES THE STATE INDEX TO REFLECT THE STATE IN THE DLV-KEYS
      *    AND UPDATES THE COURIER IF IT IS DIFFERENT IN THE UPDATED
      *    STATE
       S100-SYNC-ST-TABLE.
      ...
      *    THIS TAKES UP 6 LINES TOTAL, SO ADD THAT TO THE LINE COUNT
      ...
      *    READS DLV RECORDS UNTIL A PACKED ITEM IS FOUND
       ...
       R400-READ-BRN.
           READ BRN
           AT END
              MOVE "Y" TO WS-EOF-BRN.

       R500-READ-DLV.
           READ DLV
           AT END
              MOVE 'Y' TO WS-EOF-DLV.